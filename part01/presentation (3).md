---
## Front matter
lang: ru-RU
title: Презентация по прохождению курса Основы кибербезопасности
subtitle: курс
author:
  - Агджабекова Эся Рустамовна
institute:
  - Российский университет дружбы народов, Москва, Россия
date: 14 мая 2026

## i18n babel
babel-lang: russian
babel-otherlangs: english

## Formatting pdf
toc: false
slide_level: 2
aspectratio: 169
section-titles: true
theme: metropolis
header-includes:
 - \metroset{progressbar=frametitle,sectionpage=progressbar,numbering=fraction}
---

# Цель работы

пройти курс

# Ход выполнения работы


## Тест по теме .
  
   ![Что относится к философии свободного ПО?](images/1.png){ #fig:001 }

## Тест по теме вводный урок.
  
   ![Самая популярная оболочка](images/5.png
){ #fig:005  }

## Тест по теме роль командной стоки в серверной среде
  
   ![роль командной стоки в серверной среде](images/09.png){ #fig:009 }

## Тест по теме файловая структура и ключевые каталоги.
 
   ![ файловая структура и ключевые каталоги.](images/013.png)

){ #fig:0013 }

## Тест по теме Принцип Все есть файл
  
   ![Тест по теме Принцип Все есть файл](images/017.png)
{ #fig:0017  }
## Тест по теме роль командной стоки в серверной среде. 

   ![Тест по теме роль командной стоки в серверной среде.](images/021.png)
{ #fig:021  }

## Тест по теме Работа с базовымы командами в реальных условиях.
  

   ![Работа с базовымы командами в реальных условиях](images/027.png
){ #fig:027  }



## Тест по теме Работа с архивами и передачей данных с серверами.
 

   ![Работа с архивами и передачей данных с серверами](images/031.png
){ #fig:031}

## Тест по теме основы автоматизации с помощью Bash-скриптов.

   ![Тест по теме основы автоматизации с помощью скриптов.](images/034.png
){ #fig:0034 }

## Тест по теме основы автоматизации с помощью Bash-скриптов.

   ![Тест по теме основы автоматизации с помощью скриптов.](images/34.png
){ #fig:0034 }

# Заключение

В ходе выполнения 1 части курса Системный администратор Linux с нулябыли изучены основные принципы работы Linux
В результате работы получены практические навыки администрирования и конфигурирования Linux, необходимых для обеспечения безопасности и стабильности работы системы.

![](images/1.png)

